...
[_QWERTY] = KEYMAP( \
  ...
  TD(TD_ESC_CAPS), KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    TD(TD_GO_ASSIGN), KC_QUOT, \
 ...
 */
[_RAISE] = KEYMAP( \
  MEH_GRV, KC_EXLM, KC_AT,   KC_HASH, KC_DLR,  KC_PERC, KC_CIRC, KC_AMPR, KC_ASTR, KC_LPRN, KC_RPRN, KC_DEL, \
  RGB_TOG, RGB_HUI, RGB_SAI, RGB_VAI, _______, _______, KC_UNDS, KC_QUES, KC_PLUS, KC_LCBR, KC_RCBR, KC_PIPE, \
  RGB_MOD, RGB_HUD, RGB_SAD, RGB_VAD, _______, _______, KC_MINS, KC_SLSH, KC_EQL,  KC_LBRC, KC_RBRC, SFT_T(KC_BSLS), \
...
void go_assign_var(qk_tap_dance_state_t *state, void *user_data) {
  if (state->count == 1) {
...
qk_tap_dance_action_t tap_dance_actions[2] = {
  ...
  [TD_GO_ASSIGN] = ACTION_TAP_DANCE_FN(go_assign_var),
};
